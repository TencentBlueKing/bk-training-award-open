...
class GroupApply(TimeBasic):
    ...
    approver = models.CharField(max_length=128, blank=True, verbose_name="审批人账号")
    approver_display_name = models.CharField(max_length=128, blank=True, verbose_name="审批人姓名")

    def __str__(self):
        return f"{self.username}({self.display_name})申请加入{self.group_name}: {self.APPLY_STATE[self.status]}"
